...
tabs.forEach((el, index) => {
	el.addEventListener('click', function(e) {
		...
		const index1 = tabs.findIndex( item => item.className.includes('tab_active'));
		tabs[index1].classList.remove('tab_active');
		el.classList.toggle('tab_active');

		const index2 = tabContents.findIndex( item => item.className.includes('tab__content_active'));
